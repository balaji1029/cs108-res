...
for i in range(1,test_case_num+1):
    ...
    # command=f"./{test_file_name} ./testcases/{i}/{input_file_name} > ./testcases/{i}/{output_file_name}"
    # # print(command)
    # os.system(command)
    ...
    command=f"bash {submission_file_name} ./testcases/{i}/{input_file_name} > ./{output_dir_name}/{i}/{output_file_name}"
    # print(command)
    ...
    command=f"diff -w ./testcases/{i}/{output_file_name} ./{output_dir_name}/{i}/{output_file_name} > /dev/null 2>/dev/null "
    rval = os.system(command)
    ...
    if rval == 0:
        ...
        msg = msg + str("Output is not as Expected. Refer to testcases/") # For the failing test case, refer to /home/.evaluationScripts/autograder/testcases/")
        msg = msg + str(i) +"/ ."
    # # list to store files
    # files = []
